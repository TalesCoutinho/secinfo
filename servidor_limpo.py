...
CHUNK_SIZE = 4096
RECEIVE_DIR = Path("received")          # pasta onde os arquivos recebidos serão salvos
METRICS_FILE = Path("metrics_plain.csv")  # arquivo de métricas
...
def log_metrics(
    ...
    """
    Registra métricas de uma transferência em metrics_plain.csv.
    """
...
def handle_client(conn: socket.socket, addr) -> None:
    ...
    """
    print(f"[DEBUG] Conexão recebida de {addr}")

    # Marcar o início (monotônico para medir duração + wall clock para log)
    ...
    print(f"[DEBUG] Recebendo arquivo '{filename}' ({file_size} bytes)...")
    ...
    print(f"[DEBUG] Arquivo salvo em: {dest_path.resolve()}")
    print(f"[DEBUG] Duração da transferência: {duration:.6f} s")
    print("[DEBUG] Registrando métricas...\n")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Servidor TCP simples para receber arquivo (sem TLS)."
    )
    parser.add_argument(
        "--host",
        default="0.0.0.0",
    ...
    parser.add_argument(
        "port",
        type=int,
        help="Porta onde o servidor irá escutar (ex.: 5000)",
    )
    return parser.parse_args()


def main() -> None:
    print("[DEBUG] Iniciando servidor_limpo.py...")
    args = parse_args()
    host = args.host
    port = args.port
        ...
        while True:
            ...
            with conn:
                try:
                    ...
                    print(f"[ERRO] Ao lidar com {addr}: {e}", file=sys.stderr)
